...
class SedeRepository:
    ...
    def obtener_por_id(self, sede_id: str) -> Optional[Sede]:
        ...
        return self.db.query(Sede).filter(Sede.id == sede_id, Sede.activo == 1).first()

    def obtener_por_nombre(self, nombre: str) -> Optional[Sede]:
        """Obtener sede por nombre exacto"""
        return (
            self.db.query(Sede).filter(Sede.nombre == nombre, Sede.activo == 1).first()
        )
    ...
    def listar(
        ...
        # Aplicar filtros
        if nombre:
            query = query.filter(Sede.nombre.like(f"%{nombre}%"))  # LIKE en SQLite

        if zona_horaria:
            query = query.filter(Sede.zona_horaria == zona_horaria)
