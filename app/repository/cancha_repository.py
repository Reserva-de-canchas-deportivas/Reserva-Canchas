...
class CanchaRepository:
    ...
    def listar_por_sede(
        ...
        query = self.db.query(Cancha).filter(
            Cancha.sede_id == sede_id, Cancha.activo == 1
        )
    ...
    def actualizar(self, cancha_id: str, cancha_data: CanchaUpdate) -> Optional[Cancha]:
        ...
        for campo, valor in update_data.items():
            if campo == "activo":
                ...
                setattr(
                    cancha, campo, valor.value if hasattr(valor, "value") else valor
                )
            else:
...
def seed_canchas_demo(db: Session, sede_id: str) -> Optional[Cancha]:
    ...
    existente = (
        db.query(Cancha).filter(Cancha.sede_id == sede_id, Cancha.activo == 1).first()
    )
    if existente:
        return existente
