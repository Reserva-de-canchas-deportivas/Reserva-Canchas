...
class SlotDisponibilidad(BaseModel):
    ...
    hora_inicio: str = Field(..., description="Hora de inicio del slot (HH:MM)")

    hora_fin: str = Field(..., description="Hora de fin del slot (HH:MM)")
    ...
    motivo: Optional[str] = Field(
        None, description="Motivo por el que no es reservable (si aplica)"
    )
...
class DisponibilidadResponse(BaseModel):
    ...
    fecha: str = Field(..., description="Fecha consultada (YYYY-MM-DD)")
    ...
    zona_horaria: str = Field(..., description="Zona horaria de la sede")

    horario_apertura: Optional[str] = Field(
        None, description="Horario de apertura del día consultado"
    )

    minutos_buffer: int = Field(..., description="Minutos de buffer entre reservas")

    slots: List[SlotDisponibilidad] = Field(..., description="Lista de slots de tiempo")

    total_slots: int = Field(..., description="Total de slots en el día")

    slots_disponibles: int = Field(..., description="Número de slots disponibles")

    slots_ocupados: int = Field(..., description="Número de slots ocupados")

    dia_cerrado: bool = Field(
        default=False, description="Indica si la sede está cerrada ese día"
    )
...
class DisponibilidadQuery(BaseModel):
    ...
    fecha: str = Field(..., description="Fecha a consultar (YYYY-MM-DD)")
    ...
    def validar_fecha(cls, v: str) -> str:
        ...
        try:
            ...
            if "does not match format" in str(e):
                raise ValueError(
                    f"Formato de fecha inválido: '{v}'. " f"Debe ser YYYY-MM-DD"
                )
            raise
