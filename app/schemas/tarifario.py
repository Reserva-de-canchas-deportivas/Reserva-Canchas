...
class TarifarioCreate(BaseModel):
    ...
    sede_id: str = Field(
        ...,
        description="ID de la sede (obligatorio)"
    )
    
    cancha_id: Optional[str] = Field(
        None,
        description="ID de la cancha (opcional - null = tarifa general de sede)"
    )
    ...
    precio_por_bloque: Decimal = Field(
        ...,
        gt=0,
        description="Precio por bloque de tiempo (debe ser positivo)"
    )
    
    moneda: str = Field(
        default="COP",
        description="Código de moneda ISO 4217 (3 letras mayúsculas)"
    )
...
class TarifarioUpdate(BaseModel):
    ...
    _validar_moneda = field_validator('moneda')(
        TarifarioCreate.validar_moneda.__func__
    )
